...
from treelstm.model import SimilarityGRU,SimilarityLSTM,SimilarityBiLSTM,SimilarityBiGRU,SimilarityTreeGRU,SimilarityTreeLSTM
class DocReader(object):
    ...
    def __init__(self, args, word_dict,pos_dict,sdp_dict,state_dict=None):
        # Book-keeping.
        self.args = args
        self.word_dict = word_dict
        self.pos_dict = pos_dict
        self.sdp_dict = sdp_dict
        ...
        if args.model_type == 'lstm':
            self.network = SimilarityLSTM(args)
        elif args.model_type == 'gru':
            self.network = SimilarityGRU(args)
        elif args.model_type == 'bilstm':
            self.network = SimilarityBiLSTM(args)
        elif args.model_type == 'bigru':
            self.network = SimilarityBiGRU(args)
        elif args.model_type == 'treelstm':
            self.network = SimilarityTreeLSTM(args)
        elif args.model_type == 'treegru':
            self.network = SimilarityTreeGRU(args)
        else:
    ...
    def update(self, ex):
        ...
        if self.use_cuda:
            inputs = None#[e if e is None else Variable(e.cuda( async=True )) for e in ex[:-1]]
            targets =  None#Variable(ex[-1].cuda(async=True))
    ...
    def load(filename, new_args=None):
        ...
        pos_dict = saved_params['pos_dict']
        sdp_dict = saved_params['sdp_dict']
        state_dict = saved_params['state_dict']
        args = saved_params['args']
        if new_args:
            args = override_model_args(args, new_args)
        return DocReader(args, word_dict,pos_dict,sdp_dict,state_dict)
    ...
    def load_checkpoint(filename,args):
        ...
        args = saved_params['args']
        model = DocReader(args, word_dict,pos_dict,sdp_dict, state_dict)
